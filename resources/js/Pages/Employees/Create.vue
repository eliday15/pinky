...
const form = useForm({
    ...
    compensation_type_overrides: {},
});

// Un empleado que no checa no tiene ID ZKTeco: al marcar la casilla se limpia
// el campo para no guardar (ni bloquear) un ID que ya no aplica.
...
/** Reset schedule fields when base schedule changes. */
...
/** Build schedule_overrides from all editable state. */
const syncOverrides = () => {
    const s = selectedSchedule.value;
    if (!s) return;
    ...
    if (!perDayMode.value) {
        ...
        ['entry_time', 'exit_time'].forEach(f => {
            ...
            if (scheduleFields.value[f] && scheduleFields.value[f] !== baseVal) {
                overrides[f] = scheduleFields.value[f];
            }
        });
        ['break_minutes', 'daily_work_hours', 'late_tolerance_minutes'].forEach(f => {
            const val = scheduleFields.value[f];
            if (val !== '' && val !== null && parseFloat(val) !== parseFloat(s[f])) {
                overrides[f] = val;
            }
        });
    } else {
        ...
        if (Object.keys(ds).length) {
            overrides.day_schedules = ds;
        }
        // Still allow tolerance override
        const tol = scheduleFields.value.late_tolerance_minutes;
        if (tol !== '' && tol !== null && parseFloat(tol) !== parseFloat(s.late_tolerance_minutes)) {
            overrides.late_tolerance_minutes = tol;
        }
    }
...
const onPositionChange = () => {
    ...
    const filled = {};
    const anchor = position.anchor_employee;
    ...
    // Pre-fill hourly rate: anchor employee > position template
    if (anchor?.hourly_rate) {
        form.hourly_rate = anchor.hourly_rate;
        filled.hourly_rate = true;
    } else if (position.base_hourly_rate) {
        form.hourly_rate = position.base_hourly_rate;
        filled.hourly_rate = true;
    }

    // Pre-fill is_minimum_wage from anchor
    if (anchor && anchor.is_minimum_wage !== undefined) {
        form.is_minimum_wage = anchor.is_minimum_wage;
    }

    // Auto-fill compensation types: anchor employee > position template
    if (anchor?.compensation_types?.length) {
        form.compensation_type_ids = anchor.compensation_types.map(ct => ct.id);
        const overrides = {};
        anchor.compensation_types.forEach(ct => {
            if (ct.pivot?.custom_percentage) {
                overrides[ct.id] = ct.pivot.custom_percentage;
            } else if (ct.pivot?.custom_fixed_amount) {
                overrides[ct.id] = ct.pivot.custom_fixed_amount;
            }
        });
        form.compensation_type_overrides = overrides;
        filled.compensation_type_ids = true;
    } else if (position.compensation_types?.length) {
...
const onDepartmentChange = () => {
    if (autoFilledFields.value.compensation_type_ids) return; // position already set them
            ...
            <form @submit.prevent="submit" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                type="number"
                                :disabled="form.is_attendance_exempt"
                                class="w-full rounded-lg border-gray-300 shadow-sm focus:border-pink-500 focus:ring-pink-500 disabled:bg-gray-100 disabled:text-gray-400"
                ...
                <div class="bg-white rounded-lg shadow p-6">
                    ...
                    <div v-if="canEditAll" class="mt-6 border-t border-gray-200 pt-6">
                        ...
                        <p class="text-xs text-gray-500 mb-4">Contraseña personal que el empleado usa para cobrar su efectivo y su desayuno en el kiosco. Puede incluir letras, números y símbolos; es opcional al dar de alta.</p>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                ...
                <div v-if="canEditAll" class="bg-white rounded-lg shadow p-6">
                    ...
                    <!-- Anchor Employee Info Badge -->
                    <div v-if="selectedPosition?.anchor_employee" class="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg">
                        <p class="text-sm text-green-700">
                            <span class="font-medium">Empleado ancla:</span>
                            {{ selectedPosition.anchor_employee.full_name }}
                            — los datos de compensacion se pre-llenaron desde este empleado
                        </p>
                    </div>
                ...
                <div v-if="canEditAll" class="bg-white rounded-lg shadow p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <input
                                v-model="form.vacation_days_reserved"
                                type="number"
                                min="0"
                                class="w-full rounded-lg border-gray-300 shadow-sm focus:border-pink-500 focus:ring-pink-500"
                            />
                            <p class="mt-1 text-sm text-gray-500">
                                Dias reservados/comprometidos
                            </p>
                        </div>
                        ...
                        <div>
                            ...
                            <input
                                :value="Math.max(0, form.vacation_days_entitled - form.vacation_days_used - form.vacation_days_reserved)"
                                type="number"
                                disabled
                                class="w-full rounded-lg border-gray-300 bg-gray-50 shadow-sm"
                            />
                            <p class="mt-1 text-sm text-gray-500">
                                Correspondientes - Tomados - Apartados
                            </p>
                        </div>
                        ...
                        <div>
                            ...
                            <input
                                v-model="form.vacation_premium_percentage"
                                type="number"
                                step="0.01"
                                min="0"
                                max="100"
