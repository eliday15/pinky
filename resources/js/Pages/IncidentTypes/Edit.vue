...
const initialPositionIds = (props.incidentType.positions || []).map(p => p.id);
const initialDepartmentIds = (props.incidentType.departments || []).map(d => d.id);

const form = useForm({
    name: props.incidentType.name,
    code: props.incidentType.code,
    description: props.incidentType.description || '',
    category: props.incidentType.category || 'absence',
    is_paid: props.incidentType.is_paid,
    deducts_vacation: props.incidentType.deducts_vacation,
    requires_approval: props.incidentType.requires_approval,
...
const submit = () => {
    form.put(route('incident-types.update', props.incidentType.id));
};
</script>
