...
/**
 * Build initial position IDs and pivot overrides from existing relation.
 */
const initialPositionIds = (props.compensationType.positions || []).map(p => p.id);
const initialPositionPercentages = {};
const initialPositionFixedAmounts = {};
(props.compensationType.positions || []).forEach(p => {
    if (p.pivot?.default_percentage != null) {
        initialPositionPercentages[p.id] = p.pivot.default_percentage;
    }
    if (p.pivot?.default_fixed_amount != null) {
        initialPositionFixedAmounts[p.id] = p.pivot.default_fixed_amount;
    }
});

/**
 * Build initial department IDs and pivot overrides from existing relation.
 */
const initialDepartmentIds = (props.compensationType.departments || []).map(d => d.id);
const initialDepartmentPercentages = {};
const initialDepartmentFixedAmounts = {};
(props.compensationType.departments || []).forEach(d => {
    if (d.pivot?.default_percentage != null) {
        initialDepartmentPercentages[d.id] = d.pivot.default_percentage;
    }
    if (d.pivot?.default_fixed_amount != null) {
        initialDepartmentFixedAmounts[d.id] = d.pivot.default_fixed_amount;
    }
});
