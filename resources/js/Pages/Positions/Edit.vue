...
/**
 * Build initial compensation type IDs and overrides from the position's
 * existing compensation_types relation.
 */
const initialCompensationTypeIds = (props.position.compensation_types || []).map(ct => ct.id);
const initialPercentages = {};
const initialFixedAmounts = {};
(props.position.compensation_types || []).forEach(ct => {
    if (ct.pivot?.default_percentage !== null && ct.pivot?.default_percentage !== undefined) {
        initialPercentages[ct.id] = ct.pivot.default_percentage;
    }
    if (ct.pivot?.default_fixed_amount !== null && ct.pivot?.default_fixed_amount !== undefined) {
        initialFixedAmounts[ct.id] = ct.pivot.default_fixed_amount;
    }
});
...
const form = useForm({
    ...
    position_type: props.position.position_type,
    base_hourly_rate: props.position.base_hourly_rate,
    default_overtime_rate: props.position.default_overtime_rate,
    default_holiday_rate: props.position.default_holiday_rate,
    is_active: props.position.is_active,
    department_id: props.position.department_id,
    supervisor_position_id: props.position.supervisor_position_id || '',
    default_schedule_id: props.position.default_schedule_id || '',
    anchor_employee_id: props.position.anchor_employee_id || '',
    compensation_type_ids: initialCompensationTypeIds,
    compensation_type_percentages: initialPercentages,
    compensation_type_fixed_amounts: initialFixedAmounts,
});
...
/**
 * Filter out the current position from the supervisor position list
 * to prevent self-referencing.
 */
const availableSupervisorPositions = props.positions.filter(p => p.id !== props.position.id);
...
<template>
    <Head :title="`Editar - ${position.name}`" />
        ...
        <div class="max-w-4xl">
            ...
            <form @submit.prevent="submit" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            >
                                ...
                                <option v-for="pos in availableSupervisorPositions" :key="pos.id" :value="pos.id">
                                    {{ pos.name }}
                                </option>
                            </select>
                        ...
                        <div>
                            <label class="flex items-center space-x-3">
                                ...
                                />
                                <span class="text-sm font-medium text-gray-700">Puesto activo</span>
                            </label>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex justify-end space-x-4">
                    ...
                    >
                        {{ form.processing ? 'Guardando...' : 'Actualizar Puesto' }}
                    </button>
                </div>
            </form>
        </div>
