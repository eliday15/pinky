    ...
    <AppLayout>
        ...
        <div class="bg-white rounded-lg shadow p-4 mb-6">
            <div class="flex flex-wrap items-end gap-4">
                ...
                <a :href="route('reports.export.retardos', { start_date: dateRange.start, end_date: dateRange.end })"
        ...
        <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
            <div class="bg-white rounded-lg shadow p-4 text-center">
                <p class="text-2xl font-bold text-yellow-600">{{ summary.total_retardos }}</p>
                <p class="text-xs text-gray-500">Total Retardos</p>
            </div>
            <div class="bg-white rounded-lg shadow p-4 text-center">
                <p class="text-2xl font-bold text-gray-800">{{ summary.employees_with_retardos }}</p>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <tr v-for="row in byEmployee" :key="row.employee?.id" class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right">
                            <span :class="['px-2 py-1 rounded-full text-sm font-bold', row.late_count >= lateToAbsenceCount ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800']">
                                {{ row.late_count }}
                            </span>
                        </td>
                    ...
                    <tr v-if="byEmployee.length === 0">
                        <td colspan="5" class="px-6 py-12 text-center text-gray-500">No hay retardos registrados en este periodo</td>
                    </tr>
                </tbody>
            </table>
        </div>
        ...
        <div class="mt-6 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            <p class="text-sm text-yellow-800"><strong>Nota:</strong> {{ lateToAbsenceCount }} retardos al mes generan 1 falta automatica.</p>
        </div>
    </AppLayout>
</template>
