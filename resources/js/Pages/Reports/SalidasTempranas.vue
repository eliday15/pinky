...
const applyFilter = () => {
    router.get(route('reports.earlyDepartures'), {
...
<template>
    <Head title="Reporte de Salidas Tempranas" />
    <AppLayout>
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800">Reporte de Salidas Tempranas</h2>
        </template>
        ...
        <div class="bg-white rounded-lg shadow p-4 mb-6">
            <div class="flex flex-wrap items-end gap-4">
                ...
                <a :href="route('reports.export.earlyDepartures', { start_date: dateRange.start, end_date: dateRange.end })"
        ...
        <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
            <div class="bg-white rounded-lg shadow p-4 text-center">
                <p class="text-2xl font-bold text-orange-600">{{ summary.total_early_departures }}</p>
                <p class="text-xs text-gray-500">Total Salidas</p>
            </div>
            <div class="bg-white rounded-lg shadow p-4 text-center">
                <p class="text-2xl font-bold text-gray-800">{{ summary.employees_with_early_departures }}</p>
                <p class="text-xs text-gray-500">Empleados</p>
            </div>
            <div class="bg-white rounded-lg shadow p-4 text-center">
                <p class="text-2xl font-bold text-red-600">{{ summary.total_early_minutes }}</p>
                <p class="text-xs text-gray-500">Minutos Totales</p>
            </div>
            <div class="bg-white rounded-lg shadow p-4 text-center">
                <p class="text-2xl font-bold text-blue-600">{{ summary.avg_early_minutes }}</p>
                <p class="text-xs text-gray-500">Promedio Min.</p>
            </div>
            <div class="bg-white rounded-lg shadow p-4 text-center">
                <p class="text-2xl font-bold text-red-600">{{ summary.faltas_generated }}</p>
                <p class="text-xs text-gray-500">Generaron Falta</p>
            </div>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase">Salidas</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <tr v-for="row in byEmployee" :key="row.employee?.id" class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right">
                            <span :class="['px-2 py-1 rounded-full text-sm font-bold', row.faltas_count > 0 ? 'bg-red-100 text-red-800' : 'bg-orange-100 text-orange-800']">
                                {{ row.departure_count }}
                            </span>
                        </td>
                    ...
                    <tr v-if="byEmployee.length === 0">
                        <td colspan="5" class="px-6 py-12 text-center text-gray-500">No hay salidas tempranas registradas en este periodo</td>
                    </tr>
                </tbody>
            </table>
        </div>
        ...
        <div class="mt-6 bg-orange-50 border border-orange-200 rounded-lg p-4">
            <p class="text-sm text-orange-800">
                <strong>Nota:</strong>
                <template v-if="settings.earlyIsAbsence">
                    Salida de {{ settings.earlyThreshold }}+ min sin autorizacion genera falta. Las fechas marcadas con <strong>F</strong> cuentan como falta.
                </template>
                <template v-else>
                    Las salidas tempranas actualmente no generan faltas automaticas.
                </template>
            </p>
        </div>
    </AppLayout>
