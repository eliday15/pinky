...
/** Does the entry's [start, end] overlap the employee's regular schedule on that
 *  date? Holidays bypass the check. Returns false when we don't know enough to
 *  judge (no schedule for that day, missing times). */
const hasScheduleConflict = (entry) => {
    ...
    const entryMin = toMin(sched.entry);
    const exitMin = toMin(sched.exit);
    const startMin = toMin(entry.start_time);
    const endMin = toMin(entry.end_time);
    if (entryMin == null || exitMin == null || startMin == null || endMin == null) return false;
    return startMin < exitMin && endMin > entryMin;
};
...
const pullCopy = computed(() => {
    ...
    if (selectedPullRule.value === 'velada') {
        return {
            title: 'Veladas por Empleado',
            hint: 'Cada fila es una velada (empleado + noche). Elige un rango y carga desde checadas: se detecta una velada por cada noche trabajada en las marcas (reingreso nocturno o que cruza medianoche). Al aprobarla se genera sola la cena.',
            unit: 'velada(s)',
        };
    }
    return {
...
/** Turno de la TARDE tecleado como a.m. (Dani 2026-08-12, caso Alondra
 *  05:30–09:00 que era 17:30–21:00): corre inicio y fin +12 h con un clic.
 *  Solo se ofrece cuando ambas horas son de la mañana. */
...
/** Schedule-overlap is only meaningful for overtime/velada. Other compensation
 *  types (per_day, one_time) authorize different concepts. */
                    ...
                    <div class="flex items-start justify-between gap-3 mb-4">
                        ...
                        <div class="flex flex-col items-end gap-2 flex-shrink-0">
                            ...
                            <div class="flex gap-2">
                                <button v-if="canSuggestFromChecadas" type="button" @click="fetchBulkSuggestions"
                                ...
                                <template v-for="entry in group.entries" :key="`${entry.employee_id}_${entry.date}_${entry.kind}_${entry._index}`">
                                    ...
                                    <div v-if="isHoursType && hasScheduleConflict(entry)"
                                        ...
                                        <button
                                            ...
                                            type="button"
                                            @click="shiftEntryToPm(entry._index)"
