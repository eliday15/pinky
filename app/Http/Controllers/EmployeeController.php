...
class EmployeeController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $validated = $request->validate([
            ...
            'finiquito_amount' => ['nullable', 'numeric', 'min:0'],
            ...
            // Datos fiscales/CFDI (nómina timbrada): editables por RRHH.
            ...
            'cash_pin' => ['nullable', 'string', 'min:4', 'max:255', 'confirmed'],
            ...
            'compensation_type_overrides.*' => ['nullable', 'numeric', 'decimal:0,4', 'between:-99999999.9999,99999999.9999'],
        ...
        $query->when($filters['search'] ?? null, function ($q, $search) {
            $q->where(function ($query) use ($search) {
                $query->where('full_name', 'like', "%{$search}%")
                    ...
                    ->orWhere('zkteco_user_id', 'like', "%{$search}%")
                    ->orWhere('phone', 'like', "%{$search}%")
                    ->orWhere('credential_number', 'like', "%{$search}%")
